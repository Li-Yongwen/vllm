...
class Scheduler(SchedulerInterface):
    ...
    def _get_routed_experts(self, request: Request) -> np.ndarray | None:
        ...
        import sys
        print(f"[SCHED routed_experts] req={request.request_id} num_tokens={num_tokens} "
              f"num_blocks={len(block_ids)} block_ids[:3]={block_ids[:3]}", file=sys.stderr, flush=True)
        ...
        if compress_ratio > 1:
            ...
            block_indices = token_positions // (block_size * 4)
            offsets = token_positions % (block_size * 4)
            kv_slots = block_ids_array[block_indices] * (block_size * 4) + offsets
